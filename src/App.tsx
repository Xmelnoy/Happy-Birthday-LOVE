...
const formatTogetherTime = (date: Date) => {
  const elapsed = Date.now() - date.getTime()

  return {
    days: Math.floor(elapsed / 86_400_000),
    hours: Math.floor((elapsed / 3_600_000) % 24),
    minutes: Math.floor((elapsed / 60_000) % 60),
  }
}
...
const encodeWish = (value: string) =>
  btoa(unescape(encodeURIComponent(value)))

const decodeWish = (value: string) => {
  try {
...
const readSavedWishes = (): WishItem[] => {
  ...
  try {
    ...
    return parsed
      .map((item, index): WishItem | null => {
        if (typeof item === 'object' && item !== null) {
          const wish = item as Partial<WishItem>

          if (typeof wish.text !== 'string' || !wish.text.trim()) {
            return null
          }

          return {
            id: String(wish.id ?? `wish-${index}`),
            text: wish.text,
            completed: Boolean(wish.completed),
          }
        }
        ...
        try {
          const legacy = JSON.parse(decoded) as Partial<WishItem>

          if (typeof legacy.text === 'string' && legacy.text.trim()) {
            return {
              id: String(legacy.id ?? `wish-${index}`),
              text: legacy.text,
              completed: Boolean(legacy.completed),
            }
          }
        } catch {
          // Old versions could store plain text inside the encoded value.
        }
...
function App() {
  ...
  const [togetherTime, setTogetherTime] = useState(
    formatTogetherTime(RELATIONSHIP_START),
  )
  const [photos, setPhotos] = useState(placeholderPhotos)
  const [timeline, setTimeline] = useState(placeholderTimeline)
  const [reasons, setReasons] = useState(placeholderReasons)
  ...
  const [wishes, setWishes] = useState<WishItem[]>(readSavedWishes)
  ...
  useEffect(() => {
    localStorage.setItem(
      WISH_STORAGE_KEY,
      JSON.stringify(wishes.map((wish) => encodeWish(JSON.stringify(wish)))),
    )
  }, [wishes])
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setLoveLanguageIndex((current) => (current + 1) % loveInLanguages.length)
    }, 2000)
  ...
  const handleTimelineSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!newTimeline.date || !newTimeline.title || !newTimeline.description) {
      return
    }
    ...
    setTimeline((current) => [
      ...current,
      {
        ...
        image:
          newTimeline.image ||
          'https://picsum.photos/seed/new-moment/220/220',
      },
    ])
  ...
  const handleAddReason = () => {
    ...
    setReasons((current) => [value, ...current])
    setNewReason('')
  ...
  const handleWishSubmit = () => {
    ...
    setWishStatus('Желание сохранено ✨')
    ...
    <>
      ...
      <AnimatePresence>
        {!giftOpened && (
          ...
          >
            <div className="start-depth" aria-hidden="true">
            ...
            <div className="start-copy">
              ...
              <div className="intro-title" aria-label={heroText}>
                {heroChars.map((character, index) => (
                  <motion.span
                    key={`${character}-${index}`}
                    className={character === ' ' ? 'title-space' : undefined}
                    initial={{ opacity: 0, y: 35, rotateX: -80 }}
              ...
              >
                ...
                <br />
                <strong>Нажми на подарок — и начнём нашу историю.</strong>
              </motion.p>
            </div>
            ...
            <motion.div
              ...
              transition={{ delay: 0.55, duration: 1, type: 'spring' }}
            >
              <div className="gift-aura" />
              <div className="gift-orbit orbit-a" />
              <div className="gift-orbit orbit-b" />

              <motion.button
                type="button"
                className="gift-visual"
                whileHover={{ scale: 1.06, rotateY: -5, rotateX: 4 }}
                whileTap={{ scale: 0.94 }}
                animate={{ y: [0, -10, 0] }}
                transition={{
            ...
            <button
              type="button"
              className="music-toggle start-music"
        ...
        {giftOpened && (
          ...
          >
            ...
            <section className="hero section" id="hero">
              ...
              >
                сегодня весь мир немного красивее
              </motion.p>
              ...
              <p className="subtitle">
                Потому что сегодня родилась ты — и однажды наши дороги
                встретились.
              </p>
              ...
              <div className="counter-card">
                ...
                <p>
                  ...
                  <strong>{togetherTime.hours}</strong> часов,{' '}
                  <strong>{togetherTime.minutes}</strong> минут
                </p>
              </div>
            </section>
            ...
            >
              ...
              <p>
                [Здесь будет твой длинный, тёплый и искренний текст-послание.
                Замени этот placeholder на свою историю, воспоминания и самые
                важные слова для любимой.]
              </p>
            </motion.section>
            ...
            >
              ...
              <h2>Твои желания ✨</h2>
              <p>
                Пусть маленькие мечты превращаются в большие счастливые
                моменты. Записывай их сюда и отмечай те, что уже исполнились.
              </p>

              <button
                type="button"
                className="add-button"
                onClick={() => setWishEditorOpen((open) => !open)}
              >
                <Star size={16} />
                Добавить желание
              </button>
              ...
              <div className="wish-progress">
                <span>
                  <strong>{completedWishes}</strong> из{' '}
                  <strong>{wishes.length}</strong> исполнено
                </span>
                <div>
                  <i
                    style={{
                      width: wishes.length
                        ? `${(completedWishes / wishes.length) * 100}%`
                        : '0%',
                    }}
                  />
                </div>
              ...
              <div className="wish-list">
                {wishes.length === 0 ? (
                  <div className="wish-empty">
                    <Sparkles size={24} />
                    <span>Здесь пока пусто. Загадай первое желание.</span>
                  </div>
                ) : (
                  wishes.map((wish, index) => (
                    ...
                    >
                      <span className="wish-check">
                        {wish.completed && <Check size={15} />}
                      </span>
                      <span className="wish-text">{wish.text}</span>
                      <span className="wish-state">
                        {wish.completed ? 'исполнено' : 'отметить'}
                      </span>
                    </motion.button>
                  ))
                )}
